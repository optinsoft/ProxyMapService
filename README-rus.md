...
HostRules[].ProxyServer | Использовать этот прокси-сервер когда `Action`=`Allow`. Опционально (null если использовать прокси-сервер из массива ProxyMappings[].ProxyServers) | ProxyServer | 
