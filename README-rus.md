...
----| ---------|-----|-----------------------|
Listen | Настройки TCP подключений (см. описание `Listen` ниже) | `Listen` | отсутствует (обязательно ) |
Authentication | Параметры аутентификации клиентов | `Authentication` (см. описание ниже) | отсутствует |
ProxyServers | Массив прокси серверов (см. описание `ProxyServer` ниже) | `List<ProxyServer>` | [] |
...
Правила для маршрутизации трафика в зависимости от имени хоста задаются в разделе `HostRules`, который содержит массив элементов типа `HostRule`.
